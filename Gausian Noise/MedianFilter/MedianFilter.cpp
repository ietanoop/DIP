...
using namespace cv;
using namespace std;
void GaussianMedian(Mat,int,int);
Mat Gray(Mat);
int Median(int[],int);
...
int main()
{
  Mat image = imread("C:\\Users\\Manoj\\Desktop\\AnoopDIP\\DIP\\Gausian Noise\\MedianFilter\\test.bmp");
if (image.empty()) {
    cout << "Cannot open image!" << endl;
    return 1;
  }
Mat grayimage=Gray(image);
GaussianMedian(grayimage,25,25);
waitKey(0);
}

void GaussianMedian(Mat grayimg,int w_width,int w_height){
...
for(int i=edgeY;i<height-edgeY;i++){
    ...
    imageMean.at<uchar>(m,n++)=Median(window,w_height*w_width);
    }
  m++;n=edgeX;
}
